...
sha1_remote="$(git ls-remote origin -h refs/heads/staging|cut -f1)"
[ "`git log --pretty=%H ...refs/heads/staging^ | head -n 1`" = "`git ls-remote origin -h refs/heads/staging |cut -f1`" ] && status="current"|| status="behind"
...
if [ $status = "behind" ] ; then
    cd /rankscience/$TELENV && git fetch origin && git reset --hard origin/staging && npm install nodegit && yarn install && yarn build 
    HEAD=`git log --pretty=%H ...refs/heads/staging^ | head -n 1`
